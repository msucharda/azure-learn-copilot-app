...
- Use the selected official skill to identify topics and search terms, not final citation URLs. Resolve current Microsoft Learn URLs with a bounded `docs-search`, then fetch only the selected search results live with `record_learn_evidence` using logical operation `docs-fetch`.
- A URL embedded in a skill is only a discovery hint. Do not persist or cite it unless the current search returns that exact URL and its exact `docs-fetch` succeeds. When URL aliases produce the same fetched content, retain only the current search-result URL.
- Use `code-sample-search` only to verify SDK or code behavior.
...
Persist each bounded non-published revision with `persist_research_draft`, then open or refresh `learn-references` in `draft` view. Reuse a stable panel instance ID such as `learn-draft-panel`; it must not equal or derive from `researchId`. The canvas is reference-only: source title, section heading, exact excerpt, and canonical Microsoft Learn URL. Do not use the canvas for the research answer, claim matrix, provenance, lifecycle, or unresolved-item narrative.

After persistence, provide the complete user-facing synthesis in chat:

- lead with the conclusion and recommended architecture;
- cover the material findings, constraints, trade-offs, and unresolved risks;
- place Microsoft Learn links directly beside the claims they support;
- use only the exact `canonicalUrl` of a persisted source backed by a successful `docs-fetch` in this research;
- never link to a skill recommendation, search result, failed fetch, guessed path, or unpersisted source;
- keep exact excerpts in the reference canvas instead of repeating them in chat;
- omit routing objects, hashes, lifecycle details, and evidence plumbing unless they explain a visible failure.
