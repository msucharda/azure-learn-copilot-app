...
| The coach buries its question in JSON or a long assignment brief | Use the current shared conversation contract and remove automatic checkpoint requests from the kickoff. Ordinary turns give brief guidance and one small question; export progress only on explicit request in a separate turn. Existing multi-part library prompts must be paced across turns |
| Resuming in the same coach session asks for a checkpoint file | Continue from actual native conversation evidence. An export is needed for portable transfer, not routine continuation; re-establish only missing evidence |
