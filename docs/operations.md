...
- Persisted bundle prose is matched back to fetched content with one occurrence allocator shared by exact and character-level decorated fragments. Retention stores only covered intervals plus decorated-fragment hashes/counts. One fetched-content digest has a durable cross-version/cross-research budget of at most 12,000 characters, always shorter than the page. URL aliases do not reset the budget.
- Cumulative handoff prose is charged through immutable per-content reservations and must remain shorter than every fetched page referenced by the bundle; individual and partitioned full-page handoffs are rejected.
