...
- Persisted bundle prose counts only verified contiguous fetched-content spans: exact field values, exact excerpts, embedded matches of at least 32 characters, and insertion-only decoration alignments that preserve mandatory text runs. The aligner handles punctuation, spacing, marks, symbols, and format characters, deduplicates alternate detections within one prose range, and uses a bounded global occurrence search rather than field-order allocation. Ordinary prose that merely shares common characters with a page does not consume the budget. Hidden controls in persisted prose and hidden controls/default-ignorable characters in fetched Markdown fail validation. Retention stores only verified covered intervals. One fetched-content digest has a durable cross-version/cross-research budget of at most 12,000 characters and cannot retain 90% or more of a page. URL aliases do not reset the budget.
