...
test("MAI evaluation agents keep bounded single-purpose contracts", async () => {
    ...
    assert.match(compact(operations), /Set `plane` only from explicit source wording or permission classification/i);
    assert.match(compact(operations), /An emitted example must match its source block byte-for-byte/i);
    ...
    assert.match(compact(operations), /Selectors mirror only that invocation's exact aliases and values/i);
    assert.match(compact(operations), /never add optional flags, normalize `-s value`, or drop its value/i);
    assert.match(compact(operations), /If a protective control cannot be disabled, never recommend disabling it/i);
    ...
    assert.match(compact(operations), /Route recovery limits to `rollback_limits`/i);
    assert.match(compact(operations), /Permission target nouns must exactly match the operation target/i);
    assert.match(compact(operations), /Prerequisites are only explicit gating conditions/i);
    assert.match(compact(operations), /Compare every default\/numeric claim/i);
    assert.match(compact(operations), /generic versus tier-specific values/i);
    assert.match(compact(operations), /creation-time constraints transferred to later operations/i);
    ...
    assert.match(compact(operations), /attribution, selector and operation byte fidelity/i);
