...
test("MAI evaluation agents keep bounded single-purpose contracts", async () => {
    ...
    assert.match(compact(operations), /Selectors must mirror only the emitted invocation's exact aliases and values/i);
    assert.match(compact(operations), /do not normalize `-s value` to `--name` or drop its value/i);
    ...
    assert.match(compact(operations), /every documented action, role, and authentication alternative in `actor_permissions`/i);
    assert.match(compact(operations), /Compare every default and numeric claim across every page/i);
